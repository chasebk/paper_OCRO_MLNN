...
\ead{nguyenthieu2102@gmail.com}
\author[1]{Tu Nguyen}
\ead{nguyenanhtu3198@gmail.com}
\author[1]{Binh Minh Nguyen\corref{cor1}}
\cortext[cor1]{Corresponding author}
\ead{minhnb@soict.hust.edu.vn}
\author[2]{Giang Nguyen}
\ead{giang.ui@savba.sk}
...
For validation of the proposed model, three real-world dataset are used.
The first is "Internet traffic data~\citep{ref_cortez} collected from a private ISP (Internet Service Provider) with distributed centres in 11 European cities (in short from here EU dataset). 
The second dataset is the WorldCup98 (in short from here WC dataset) contains request numbers to server in world-cup season in 1998. 
The third dataset is gathered by Google from their production data center clusters (in short from here Google dataset). The log records come from approximately 12000 servers during one month~\citep{ref_google_trace}, \citep{reiss2011google}.


%In this paper, we propose a new algorithm with an improvement of coral reefs optimization using opposition-based learning (OCRO). To explore hidden relationships in the non-linear time series data, we also propose a novel time series forecasting model which is combined the opposition-based coral reefs optimization the multi-layer neural network (OCRO-MLNN). This also enables our prediction model to reduce the complexity of the model and also fast convergence than when using backpropagation algorithm as the traditional model. We use three kind of real-world dataset which are internet traffic data (in megabytes) from EU cities in 2005 (univariate data), request to server in worldcup season in 1998 (univariate data), and resource usages from Google trace in 2011 (multivariate data) to test our proposed forecasting model. The gained results demonstrate that our model can work effectively in real situations with good performance.
\end{abstract}
...
%presupposing the linear form of the model which is a linearly correlated structure is assumed among the time series values ​​and by that the ARIMA model can not capture non-linear patterns, so the approximation of the linear model to complex real-world problems is almost inadequate.
...
$N \times M$ & Number of square grid in reef \\ 		%\hline
$\rho_{o}$ & The rate between free/occupied squares in reef at the begining of the algorithm \\ 		%\hline
...
$\tau$ & Number of iters before restarting algorithm \\ 					%\hline
\end{tabular}
\end{center}
...
%Due to required of training epoch time is very large (for example, with $k = 2$, RNN's $t_e$ = 2.1575 seconds and LSTM's $t_e$ = 1.8913 seconds), then RNN and LSTM system's time become very large (with $k = 2$, RNN's $t_s$ = 2158.05 and LSTM's $t_s$ = 1891.72). This caused by the complicated structure of these  model and considered to be harmful to physical machine. 
...
%For the traditional MLNN's training time per epoch is significant small compare to others but slow convergence causes this model need more time on training process. 
...
%For visualization of this test, we decribes the RMSE range with after 15 run times of comparison models using using box-and-whisker plots in Fig.~\ref{fig:stability_uni}. While the intermittent lines in the middle of boxes represent the mean values, the upper and lower boundaries of the boxes represents upper and lower quantiles of the distributions. 
